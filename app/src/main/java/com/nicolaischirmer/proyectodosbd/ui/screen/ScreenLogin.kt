...
import com.nicolaischirmer.proyectodosbd.data.AuthRes
import com.nicolaischirmer.proyectodosbd.ui.theme.Purple40
...
fun LoginScreen(
    ...
    Box(modifier = Modifier.fillMaxSize()){
        ...
        Column (
            ...
            Text(text = "-------- or --------", style = TextStyle(color = Color.Gray))
            Spacer(modifier = Modifier.height(25.dp))
            SocialMediaButton(
                onClick = {
                    scope.launch {
...
suspend fun signIn(email: String, password: String, context: Context, auth: AuthManager, navigateToHome: () -> Unit) {
    if (email.isNotEmpty() && password.isNotEmpty()) {
        ...
        Toast.makeText(context, "Email and password have to be filled", Toast.LENGTH_SHORT).show()
    }
}
